...
char const* H264VideoRTPSink::auxSDPLine() {
  ...
  if (sps == NULL || pps == NULL) {
    ...
    if (fOurFragmenter == NULL) return VideoRTPSink::auxSDPLine(); // we don't yet have a fragmenter (and therefore not a source)
    ...
    if (sps == NULL || pps == NULL) return VideoRTPSink::auxSDPLine(); // our source isn't ready
  }
  ...
  if (spsWEBSize < 4) { // Bad SPS size => assume our source isn't ready
    delete[] spsWEB;
    return VideoRTPSink::auxSDPLine();
  }
