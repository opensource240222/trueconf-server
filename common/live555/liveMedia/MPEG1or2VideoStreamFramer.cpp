...
enum MPEG1or2ParseState {
  MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER,
  MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER_SEEN_CODE,
  MPEG1OR2_PARSING_GOP_HEADER,
  MPEG1OR2_PARSING_GOP_HEADER_SEEN_CODE,
  MPEG1OR2_PARSING_PICTURE_HEADER,
  MPEG1OR2_PARSING_SLICE
};
...
private:
  ...
  void setParseState(MPEG1or2ParseState parseState);
...
private:
  MPEG1or2ParseState fCurrentParseState;
...
::MPEG1or2VideoStreamParser(MPEG1or2VideoStreamFramer* usingSource,
  ...
  : MPEGVideoStreamParser(usingSource, inputSource),
    fCurrentParseState(MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER),
...
void MPEG1or2VideoStreamParser::setParseState(MPEG1or2ParseState parseState) {
...
void MPEG1or2VideoStreamParser::flushInput() {
  ...
  if (fCurrentParseState != MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER) {
    setParseState(MPEG1OR2_PARSING_GOP_HEADER); // start from the next GOP
  }
}
...
unsigned MPEG1or2VideoStreamParser::parse() {
  try {
    ...
    case MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER: {
      return parseVideoSequenceHeader(False);
    }
    case MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER_SEEN_CODE: {
      return parseVideoSequenceHeader(True);
    }
    case MPEG1OR2_PARSING_GOP_HEADER: {
      return parseGOPHeader(False);
    }
    case MPEG1OR2_PARSING_GOP_HEADER_SEEN_CODE: {
      return parseGOPHeader(True);
    }
    case MPEG1OR2_PARSING_PICTURE_HEADER: {
      return parsePictureHeader();
    }
    case MPEG1OR2_PARSING_SLICE: {
      return parseSlice();
    }
    default: {
...
extern double const frameRateFromCode[] = {
  0.0,          // forbidden
...
::parseVideoSequenceHeader(Boolean haveSeenStartCode) {
...
#endif
      get1Byte(); setParseState(MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER);
  ...
  setParseState((next4Bytes == GROUP_START_CODE)
		? MPEG1OR2_PARSING_GOP_HEADER_SEEN_CODE : MPEG1OR2_PARSING_PICTURE_HEADER);
...
unsigned MPEG1or2VideoStreamParser::parseGOPHeader(Boolean haveSeenStartCode) {
...
#endif
      get1Byte(); setParseState(MPEG1OR2_PARSING_GOP_HEADER);
          // ensures we progress over bad data
    }
    first4Bytes = get4Bytes();
  } else {
  ...
  setParseState(MPEG1OR2_PARSING_PICTURE_HEADER);
...
unsigned MPEG1or2VideoStreamParser::parsePictureHeader() {
  ...
  setParseState(MPEG1OR2_PARSING_SLICE);
...
unsigned MPEG1or2VideoStreamParser::parseSlice() {
  ...
  if (isSliceStartCode(next4Bytes)) { // common case
    ...
    case SEQUENCE_END_CODE: {
      setParseState(MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER);
      break;
    }
    case VIDEO_SEQUENCE_HEADER_START_CODE: {
      setParseState(MPEG1OR2_PARSING_VIDEO_SEQUENCE_HEADER_SEEN_CODE);
      break;
    }
    case GROUP_START_CODE: {
      setParseState(MPEG1OR2_PARSING_GOP_HEADER_SEEN_CODE);
      break;
    }
    ...
    default: {
      usingSource()->envir() << "MPEG1or2VideoStreamParser::parseSlice(): Saw unexpected code "
			    << (void*)next4Bytes << "\n";
      setParseState(MPEG1OR2_PARSING_SLICE); // the safest way to recover...
      break;
    }
    }
  }
...
#undef VSH_MAX_SIZE
#undef VIDEO_SEQUENCE_HEADER_START_CODE
#undef GROUP_START_CODE
#undef PICTURE_START_CODE
#undef SEQUENCE_END_CODE
