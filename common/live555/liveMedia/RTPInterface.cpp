...
Boolean RTPInterface::sendRTPorRTCPPacketOverTCP(u_int8_t* packet, unsigned packetSize,
  ...
  // Copy packet to temporary buffer to write it using only one send() to avoid resorting to
  // blocking I/O as much as possible.
  u_int8_t* buffer = NULL;
  do {
    u_int8_t* buffer = new u_int8_t[4 + packetSize];
    buffer[0] = '$';
    buffer[1] = streamChannelId;
    buffer[2] = (u_int8_t) ((packetSize&0xFF00)>>8);
    buffer[3] = (u_int8_t) (packetSize&0xFF);
    memcpy(buffer + 4, packet, packetSize);
    if (!sendDataOverTCP(socketNum, buffer, 4 + packetSize, False)) break;
#ifdef DEBUG_SEND
  ...
  envir().setResultErrMsg("RTP/RTCP over TCP failed: ");
#ifdef DEBUG_SEND
...
Boolean RTPInterface::sendDataOverTCP(int socketNum, u_int8_t const* data, unsigned dataSize, Boolean forceSendToSucceed) {
  ...
  if (sendResult < (int)dataSize) {
    ...
    if (numBytesSentSoFar > 0 || (forceSendToSucceed && envir().getErrno() == EAGAIN)) {
      ...
      if (sendResult < (int)numBytesRemainingToSend) {
...
#endif
	envir().setResultErrMsg("send failed (fatal): ");
	...
	envir().setResultMsg("send failed: unknown error");
	return False;
    }
  }
