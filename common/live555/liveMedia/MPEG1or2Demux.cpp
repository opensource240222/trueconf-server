...
enum MPEGPSParseState {
  MPEGPS_PARSING_PACK_HEADER,
  MPEGPS_PARSING_SYSTEM_HEADER,
  MPEGPS_PARSING_PES_PACKET
};
...
private:
  void setParseState(MPEGPSParseState parseState);
...
private:
  ...
  MPEGPSParseState fCurrentParseState;
};
...
MPEGProgramStreamParser::MPEGProgramStreamParser(MPEG1or2Demux* usingDemux,
  ...
  fUsingDemux(usingDemux), fCurrentParseState(MPEGPS_PARSING_PACK_HEADER) {
}

MPEGProgramStreamParser::~MPEGProgramStreamParser() {
}

void MPEGProgramStreamParser::setParseState(MPEGPSParseState parseState) {
...
unsigned char MPEGProgramStreamParser::parse() {
  ...
  try {
    do {
      ...
      case MPEGPS_PARSING_PACK_HEADER: {
	parsePackHeader();
	break;
      }
      case MPEGPS_PARSING_SYSTEM_HEADER: {
	parseSystemHeader();
	break;
      }
      case MPEGPS_PARSING_PES_PACKET: {
...
#endif
      setParseState(MPEGPS_PARSING_SYSTEM_HEADER);
      return;
    } else if (isPacketStartCode(first4Bytes)) {
#ifdef DEBUG
      fprintf(stderr, "found packet start code 0x%02x instead of pack header\n", first4Bytes);
#endif
      setParseState(MPEGPS_PARSING_PES_PACKET);
      return;
    }

    setParseState(MPEGPS_PARSING_PACK_HEADER); // ensures we progress over bad data
...
unsigned char MPEGProgramStreamParser::parsePESPacket() {
  ...
  if (!isPacketStartCode(next4Bytes)) {
    ...
    setParseState(MPEGPS_PARSING_PACK_HEADER);
    return 0;
  }
...
#undef RAW_PES
#undef PACK_START_CODE
#undef SYSTEM_HEADER_START_CODE
#undef PACKET_START_CODE_PREFIX
#undef private_stream_1
#undef private_stream_2
#undef READER_NOT_READY
