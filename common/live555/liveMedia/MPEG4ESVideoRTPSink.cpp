...
::MPEG4ESVideoRTPSink(UsageEnvironment& env, Groupsock* RTPgs, unsigned char rtpPayloadFormat, u_int32_t rtpTimestampFrequency,
  ...
  : VideoRTPSink(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, "MP4V-ES"),
    fVOPIsPresent(False), fProfileAndLevelIndication(profileAndLevelIndication), fAuxSDPLine(NULL) {
...
char const* MPEG4ESVideoRTPSink::auxSDPLine() {
  ...
  if (fProfileAndLevelIndication == 0 || config == NULL) {
    ...
    MPEG4VideoStreamFramer* framerSource = (MPEG4VideoStreamFramer*)fSource;
    if (framerSource == NULL) return VideoRTPSink::auxSDPLine(); // we don't yet have a source

    fProfileAndLevelIndication = framerSource->profile_and_level_indication();
    if (fProfileAndLevelIndication == 0) return VideoRTPSink::auxSDPLine(); // our source isn't ready

    config = framerSource->getConfigBytes(configLength);
    if (config == NULL) return VideoRTPSink::auxSDPLine(); // our source isn't ready
  }

  char const* baseAuxSDP = VideoRTPSink::auxSDPLine();
  if (baseAuxSDP == NULL) baseAuxSDP = "";
  char const* auxSDPFmt = "%s"
    "a=fmtp:%d "
    "profile-level-id=%d;"
    "config=";
  unsigned auxSDPFmtSize = strlen(auxSDPFmt) + strlen(baseAuxSDP)
    + 3 /* max char len */
    + 3 /* max char len */
    + 2*configLength /* 2*, because each byte prints as 2 chars */
    + 2 /* trailing \r\n */;
  char* auxSDP = new char[auxSDPFmtSize];
  sprintf(auxSDP, auxSDPFmt, baseAuxSDP,
	  rtpPayloadType(),
	  fProfileAndLevelIndication);
  char* endPtr = &auxSDP[strlen(auxSDP)];
  ...
  delete[] fAuxSDPLine; fAuxSDPLine = auxSDP;
  return fAuxSDPLine;
}
