...
OnDemandServerMediaSubsession::sdpLines() {
  if (fLastStreamToken != NULL) {
    // We are already streaming, so we can easily update our SDP using existing sink.
    StreamState* state = (StreamState*)fLastStreamToken;
    setSDPLinesFromRTPSink(state->rtpSink(), state->mediaSource(), state->rtpSink()->estimatedBitrate());
  } else {
