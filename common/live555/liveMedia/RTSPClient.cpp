...
Boolean RTSPClient::setRequestFields(RequestRecord* request,
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0) {
    ...
    char const* transportFmt = "Transport: %s%s%s%s\r\n";
    char const* transportSpec;
    if (strcmp(subsession.protocolName(), "UDP") == 0) {
      suffix = "";
      transportSpec = "RAW/RAW/UDP";
    } else if (streamUsingTCP) {
      transportSpec = "RTP/AVP/TCP";
    } else {
    ...
    char* portStr;
    if (portFmt) {
      unsigned portSize = strlen(portFmt) + 2*5 /* max port len */;
      portStr = new char[portSize];
      sprintf(portStr, portFmt, rtpNumber, rtcpNumber);
    } else {
      portStr = strDup("");
    }

    unsigned transportSize = strlen(transportFmt)
      + strlen(transportSpec) + strlen(transportTypeStr) + strlen(modeStr) + strlen(portStr);
    char* transportStr = new char[transportSize];
    sprintf(transportStr, transportFmt,
	    transportSpec, transportTypeStr, modeStr, portStr);
    delete[] portStr;
...
static char* getLineRTSP(char* startOfLine) {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
	...
	nextLineStart = getLineRTSP(lineStart);
