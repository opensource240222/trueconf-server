...
OggFileParser::OggFileParser(OggFile& ourFile, FramedSource* inputSource,
  ...
  if (ourDemux == NULL) {
    // Initialization
    fCurrentParseState = OGG_PARSING_START_OF_FILE;
    continueParsing();
  } else {
    fCurrentParseState = OGG_PARSING_AND_DELIVERING_PAGES;
  ...
  try {
    while (1) {
      switch (fCurrentParseState) {
        case OGG_PARSING_START_OF_FILE: {
	  if (parseStartOfFile()) return True;
	}
        case OGG_PARSING_AND_DELIVERING_PAGES: {
	  parseAndDeliverPages();
        }
        case OGG_DELIVERING_PACKET_WITHIN_PAGE: {
...
Boolean OggFileParser::parseAndDeliverPage() {
  ...
  fCurrentParseState = OGG_DELIVERING_PACKET_WITHIN_PAGE;
  saveParserState();
  return False;
}
