...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  : RTPSink(env, rtpGS, rtpPayloadType, rtpTimestampFrequency,
    ...
    fSendTimeout(0), fSendRetryDelay(0) {
#ifdef DEBUG_RESEND
  fRetriesMade = 0;
  memset(fRetriesDistributionLost, 0, sizeof(fRetriesDistributionLost));
  memset(fRetriesDistributionAll, 0, sizeof(fRetriesDistributionAll));
#endif
...
void MultiFramedRTPSink::sendPacketIfNecessary() {
  if (fNumFramesUsedSoFar > 0) {
	...
	if (fSendTimeout > 0) {
	  ...
	  // if fMostRecentPresentationTime + fSendTimeout > timeNow + fSendRetryDelay
	  if ((fMostRecentPresentationTime.tv_sec  - timeNow.tv_sec)*1000000
	    + (fMostRecentPresentationTime.tv_usec - timeNow.tv_usec)
	    + (long)fSendTimeout - (long)fSendRetryDelay > 0) {
#ifdef DEBUG_RESEND
	    ++fRetriesMade;
#endif
	    nextTask() = envir().taskScheduler().scheduleDelayedTask(fSendRetryDelay, (TaskFunc*)retrySend, this);
	    return;
	  }
	}
#ifdef DEBUG_RESEND
	++fRetriesDistributionLost[fRetriesMade < DEBUG_RESEND_DISTRIBUTION_SIZE ? fRetriesMade : DEBUG_RESEND_DISTRIBUTION_SIZE-1];
#endif
	// if failure handler has been specified, call it
	if (fOnSendErrorFunc != NULL) (*fOnSendErrorFunc)(fOnSendErrorData);
      }
#ifdef DEBUG_RESEND
      ++fRetriesDistributionAll[fRetriesMade < DEBUG_RESEND_DISTRIBUTION_SIZE ? fRetriesMade : DEBUG_RESEND_DISTRIBUTION_SIZE-1];
    if (fPacketCount % 1000 == 0) {
      unsigned totalLost = 0;
      envir() << "resends:\n\t#\tlost\tall\t%\n";
      for (int i = 0; i < DEBUG_RESEND_DISTRIBUTION_SIZE; ++i) {
	envir() << "\t" << i << "\t" << fRetriesDistributionLost[i] << "\t" <<fRetriesDistributionAll[i] << "\t" << ((float)fRetriesDistributionLost[i]/fRetriesDistributionAll[i])*100 << "%\n";
	totalLost += fRetriesDistributionLost[i];
      }
      envir() << "total lost: " << totalLost << "/" << fPacketCount << " = " << ((float)totalLost/fPacketCount)*100 << "%\n";
    }
    fRetriesMade = 0;
#endif
    ++fPacketCount;
...
void MultiFramedRTPSink::retrySend(void* clientData) {
  MultiFramedRTPSink* sink = (MultiFramedRTPSink*)clientData;
