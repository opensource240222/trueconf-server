...
void MPEG4GenericRTPSink::setXSampleRate(unsigned value)
{
  ...
  const char auxSDPFmt[] = "a=x-samplerate:%u\r\n%s";
  unsigned auxSDPFmtSize = sizeof(auxSDPFmt) + 10-2/*sample rate*/ + strlen(fFmtpSDPLine)-2 + 1;
  fAuxSDPLine = new char[auxSDPFmtSize];
  sprintf(fAuxSDPLine, auxSDPFmt, fSampleRate, fFmtpSDPLine);
}

char const* MPEG4GenericRTPSink::auxSDPLine() {
  return fAuxSDPLine ? fAuxSDPLine : fFmtpSDPLine;
}
