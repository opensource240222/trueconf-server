...
static unsigned const uSecsPerFrameQCELP = 20000; // 20 ms
...
void QCELPDeinterleaver::doGetNextFrame() {
  ...
  if (fDeinterleavingBuffer->retrieveFrame(fTo, fMaxSize,
    ...
    fDurationInMicroseconds = uSecsPerFrameQCELP;
...
::deliverIncomingFrame(unsigned frameSize,
  ...
  unsigned uSecIncrement = (frameIndex-1)*(interleaveL+1)*uSecsPerFrameQCELP;
...
::retrieveFrame(unsigned char* to, unsigned maxSize,
  ...
  if (fromSize == 0) {
    ...
    resultPresentationTime.tv_usec += uSecsPerFrameQCELP;
...
#undef QCELP_MAX_FRAME_SIZE
#undef QCELP_MAX_INTERLEAVE_L
#undef QCELP_MAX_FRAMES_PER_PACKET
#undef QCELP_MAX_INTERLEAVE_GROUP_SIZE
