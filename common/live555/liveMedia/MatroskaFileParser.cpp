...
MatroskaFileParser::MatroskaFileParser(MatroskaFile& ourFile, FramedSource* inputSource,
  ...
  if (ourDemux == NULL) {
    // Initialization
    fCurrentParseState = MKV_PARSING_START_OF_FILE;
    continueParsing();
  } else {
    fCurrentParseState = MKV_LOOKING_FOR_CLUSTER;
...
void MatroskaFileParser::seekToTime(double& seekNPT) {
  ...
  if (seekNPT <= 0.0) {
    ...
    fCurrentParseState = MKV_LOOKING_FOR_BLOCK;
  ...
  try {
    ...
    do {
      switch (fCurrentParseState) {
        case MKV_PARSING_START_OF_FILE: {
	  areDone = parseStartOfFile();
	  break;
	}
        case MKV_LOOKING_FOR_TRACKS: {
	  lookForNextTrack();
	  break;
	}
        case MKV_PARSING_TRACK: {
	  areDone = parseTrack();
	  if (areDone && fOurFile.fCuesOffset > 0) {
	    ...
	    fCurrentParseState = MKV_PARSING_CUES;
	    areDone = False;
	  }
	  break;
	}
        case MKV_PARSING_CUES: {
	  areDone = parseCues();
	  break;
	}
        case MKV_LOOKING_FOR_CLUSTER: {
	  ...
	  break;
        ...
        case MKV_LOOKING_FOR_BLOCK: {
	  lookForNextBlock();
	  break;
	}
        case MKV_PARSING_BLOCK: {
	  parseBlock();
	  break;
	}
        case MKV_DELIVERING_FRAME_WITHIN_BLOCK: {
	  if (!deliverFrameWithinBlock()) return False;
	  break;
	}
        case MKV_DELIVERING_FRAME_BYTES: {
	  deliverFrameBytes();
...
Boolean MatroskaFileParser::parseStartOfFile() {
  ...
  fCurrentParseState = MKV_LOOKING_FOR_TRACKS;
  skipHeader(size);
...
void MatroskaFileParser::lookForNextTrack() {
  ...
  while (fCurrentParseState == MKV_LOOKING_FOR_TRACKS) {
	...
	fCurrentParseState = MKV_PARSING_TRACK;
	break;
      }
...
void MatroskaFileParser::lookForNextBlock() {
  ...
  EBMLId id;
  EBMLDataSize size;
  while (fCurrentParseState == MKV_LOOKING_FOR_BLOCK) {
    while (!parseEBMLIdAndSize(id, size)) {}
#ifdef DEBUG
	...
	fCurrentParseState = MKV_PARSING_BLOCK;
	break;
      }
...
void MatroskaFileParser::parseBlock() {
  ...
  do {
    ...
    fCurrentParseState = MKV_DELIVERING_FRAME_WITHIN_BLOCK;
...
Boolean MatroskaFileParser::deliverFrameWithinBlock() {
  ...
  do {
    ...
    if (specialFrameSource != NULL) {
      ...
      fCurrentParseState = MKV_DELIVERING_FRAME_BYTES;
      setParseState();
    }
    return True;
  } while (0);
