...
public:
  ...
  void enableSendRetries(unsigned timeout, unsigned retryDelay = 5000) {
    // Enables retries after failed packet send (disabled by default).
    // Note that this *will* result in duplicate RTP packets sent if this sink sends to multiple destinations.
    // timeout == 0 disables this option.
    fSendTimeout = timeout;
    fSendRetryDelay = retryDelay;
  }
...
private:
  ...
  static void retrySend(void* clientData);
...
private:
  ...
  unsigned fSendRetryDelay;
#ifdef DEBUG_RESEND
#define DEBUG_RESEND_DISTRIBUTION_SIZE 11
  unsigned fRetriesMade;
  unsigned fRetriesDistributionLost[DEBUG_RESEND_DISTRIBUTION_SIZE];
  unsigned fRetriesDistributionAll[DEBUG_RESEND_DISTRIBUTION_SIZE];
#endif
};
