...
LIVE_API FILE* OpenInputFile(UsageEnvironment& env, char const* fileName);

LIVE_API void CloseInputFile(FILE* fid);

LIVE_API u_int64_t GetFileSize(char const* fileName, FILE* fid);
    // 0 means zero-length, unbounded, or unknown

LIVE_API int64_t SeekFile64(FILE *fid, int64_t offset, int whence);
    // A platform-independent routine for seeking within (possibly) large files

LIVE_API int64_t TellFile64(FILE *fid);
    // A platform-independent routine for reporting the position within
    // (possibly) large files

LIVE_API Boolean FileIsSeekable(FILE *fid);
