...
char const* AMRAudioRTPSink::auxSDPLine() {
  char const* baseAuxSDP = AudioRTPSink::auxSDPLine();
  if (baseAuxSDP == NULL) baseAuxSDP = "";
  // Generate a "a=fmtp:" line with "octet-aligned=1"
  // (That is the only non-default parameter.)
  char const* auxSDPFmt = "%sa=fmtp:%d octet-align=1\r\n";
  unsigned auxSDPFmtSize = strlen(auxSDPFmt) + strlen(baseAuxSDP) + 3 /* max char len */;
  char* auxSDP = new char[auxSDPFmtSize];
  sprintf(auxSDP, auxSDPFmt, baseAuxSDP, rtpPayloadType());
