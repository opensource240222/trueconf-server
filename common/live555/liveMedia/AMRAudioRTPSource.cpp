...
static unsigned const uSecsPerFrameAMR = 20000; // 20 ms
...
void AMRDeinterleaver::doGetNextFrame() {
  ...
  if (fDeinterleavingBuffer->retrieveFrame(fTo, fMaxSize,
    ...
    fDurationInMicroseconds = uSecsPerFrameAMR;
...
::deliverIncomingFrame(unsigned frameSize, RawAMRRTPSource* source,
  ...
  unsigned uSecIncrement = frameBlockIndex*(fILL+1)*uSecsPerFrameAMR;
...
::retrieveFrame(unsigned char* to, unsigned maxSize,
  ...
  if (fromSize == 0) {
    ...
    resultPresentationTime.tv_usec += uSecsPerFrameAMR;
...
#undef MAX_INTERLEAVING_GROUP_SIZE
#undef FT_SPEECH_LOST
#undef FT_NO_DATA
#undef FT_INVALID
#undef AMR_MAX_FRAME_SIZE
