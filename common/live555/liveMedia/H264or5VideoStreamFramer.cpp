...
unsigned H264or5VideoStreamParser::parse() {
  try {
	...
	if (!fHaveSeenFirstByteOfNALUnit) {
	  ...
	  fHaveSeenFirstByteOfNALUnit = True;
	  if (usingSource()->fUseSourcePresentationTime)
	    usingSource()->fPresentationTime = sourcePresentationTime();
	}
      }
      while (next4Bytes != 0x00000001 && (next4Bytes&0xFFFFFF00) != 0x00000100) {
    ...
    if (!usingSource()->fUseSourcePresentationTime)
      usingSource()->fPresentationTime = usingSource()->fNextPresentationTime;
#ifdef DEBUG
...
#undef VPS_MAX_SIZE
#undef SPS_MAX_SIZE
#undef PREFIX_SEI_NUT
#undef SUFFIX_SEI_NUT
#undef DEBUG_PRINT_TABS
#undef DEBUG_PRINT
#undef DEBUG_STR
#undef DEBUG_TAB
#undef SEI_MAX_SIZE
#undef MAX_SEI_PAYLOAD_TYPE_DESCRIPTION_H264
#undef NUM_NEXT_SLICE_HEADER_BYTES_TO_ANALYZE
