...
void BasicUsageEnvironment0::prependToResultMsg(MsgString msg) {
  unsigned spaceAvailable = fBufferMaxSize - fCurBufferSize;
  unsigned msgLength = strlen(msg);

  if (msgLength >= fBufferMaxSize-1) {
    // There is no room for current message, so forget about it and copy what we can from "msg"
    msgLength = fBufferMaxSize-1;
  } else {
    if (msgLength > spaceAvailable-1) {
      // Truncate current message if needed
      fCurBufferSize = fBufferMaxSize-1-msgLength;
    }
    // Shift current message forward
    memmove(fResultMsgBuffer+msgLength, fResultMsgBuffer, fCurBufferSize);
  }
  memmove(fResultMsgBuffer, msg, msgLength);
  fCurBufferSize += msgLength;
  fResultMsgBuffer[fCurBufferSize] = '\0';
}
