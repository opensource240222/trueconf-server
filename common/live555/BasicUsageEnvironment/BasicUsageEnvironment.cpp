...
UsageEnvironment& BasicUsageEnvironment::operator<<(struct timeval tv) {
  if (tv.tv_sec >= 0)
    fprintf(stderr, "%li.%.6li", tv.tv_sec, tv.tv_usec);
  else
    fprintf(stderr, "%li.%.6li", tv.tv_sec+1, 1000000-tv.tv_usec);
  return *this;
}
