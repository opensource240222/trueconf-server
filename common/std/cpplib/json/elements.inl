...
public:
   ...
   Imp* Clone() const override { return new Imp_T<ElementTypeT>(*this); }
   ...
   bool Compare(const Imp& imp) const override
   {
...
class UnknownElement::ConstCastVisitor : public ConstVisitor
{
   ...
   void Visit(const Boolean& ) override {}
...
public:
   ...
   void Visit(const ElementTypeT& element) override { m_pElement = &element; } // we don't know what this is, but it overrides one of the base's no-op functions
...
public:
   ...
   void Visit(ElementTypeT& element) override { m_pElement = &element; } // we don't know what this is, but it overrides one of the base's no-op functions
...
/* std::list from GCC 4.7 doesn't have overload for const_iterator.
 * Because there is no way around that, this function is removed.
inline Object::const_iterator Object::Erase(const_iterator itWhere)
{
	return m_Members.erase(itWhere);
}
*/
...
inline bool Null::operator == (const Null& ) const
{
   return true;
}
