...
class FilterDumpVideo : public AbstractFilter<AbstractSingleSourceSink>
{
public:
	static std::shared_ptr<AbstractSource> Create(const std::shared_ptr<AbstractSource>& src, string_view name);

#if FFL_DUMP_VIDEO
	FilterDumpVideo(string_view name);
...
#if !FFL_DUMP_VIDEO
inline std::shared_ptr<AbstractSource> FilterDumpVideo::Create(const std::shared_ptr<AbstractSource>& src, string_view /*name*/)
{
	return src;
}
#endif
