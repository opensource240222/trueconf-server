                                          ...
                                          WCHAR* pcDirectXVersionLetter )
{
    ...
    if( GetSystemDirectoryW( szPath, MAX_PATH ) != 0 )
    {
        szPath[MAX_PATH - 1] = 0;
...
HRESULT GetFileVersion( WCHAR* szPath, ULARGE_INTEGER* pllFileVersion )
{
    ...
    DWORD dwHandle;
    UINT cb;
    cb = GetFileVersionInfoSizeW( szPath, &dwHandle );
    if( cb > 0 )
    {
        ...
        if( GetFileVersionInfoW( szPath, 0, cb, pFileVersionBuffer ) )
        {
            VS_FIXEDFILEINFO* pVersion = NULL;
            if( VerQueryValueW( pFileVersionBuffer, L"\\", ( VOID** )&pVersion, &cb ) &&
                pVersion != NULL )
            {
...
void CorrectDXVersion(DWORD *DXVersionMajor, DWORD *DXVersionMinor)
{
	OSVERSIONINFOA osvi;
...
{
	HRESULT res;
	WCHAR pcDirectXVersionLetter[16] = {0};
	res = GetDXVersion(DXVersionMajor, DXVersionMinor, pcDirectXVersionLetter);
	if (res == S_OK && *DXVersionMajor >= 10) {
		CorrectDXVersion(DXVersionMajor, DXVersionMinor);
	}
	return res;
}
...
{
	DWORD DXVersionMajor = 0, DXVersionMinor = 0;
	VSGetDXVersion(&DXVersionMajor, &DXVersionMinor);
	return (DXVersionMajor >= 10);
}
