...
namespace webrtc
{
	...
	class VS_HighPassFilterImpl::BiquadFilter
	{
	public:
	...
	VS_HighPassFilterImpl::VS_HighPassFilterImpl(rtc::CriticalSection* crit)
		: crit_(crit)
	{
		RTC_DCHECK(crit_);
	}

	VS_HighPassFilterImpl::~VS_HighPassFilterImpl() {}

	void VS_HighPassFilterImpl::Initialize(size_t channels, int sample_rate_hz)
	{
	...
	void VS_HighPassFilterImpl::ProcessCaptureAudio(AudioBuffer* audio)
	{
		RTC_DCHECK(audio);
		rtc::CritScope cs(crit_);
		if (!enabled_)
	...
	int VS_HighPassFilterImpl::Enable(bool enable)
	{
		rtc::CritScope cs(crit_);
		if (!enabled_ && enable)
		{
	...
	bool VS_HighPassFilterImpl::is_enabled() const
	{
		rtc::CritScope cs(crit_);
		return enabled_;
	}
