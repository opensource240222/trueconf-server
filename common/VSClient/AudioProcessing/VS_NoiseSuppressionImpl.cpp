...
namespace webrtc
{
	class VS_NoiseSuppressionImpl::Suppressor
	{
	public:
	...
	VS_NoiseSuppressionImpl::VS_NoiseSuppressionImpl(rtc::CriticalSection* crit)
		: crit_(crit)
	{
		RTC_DCHECK(crit);
	}

	VS_NoiseSuppressionImpl::~VS_NoiseSuppressionImpl()
	{
	}

	void VS_NoiseSuppressionImpl::Initialize(size_t channels, int sample_rate_hz)
	{
	...
	void VS_NoiseSuppressionImpl::AnalyzeCaptureAudio(AudioBuffer* audio)
	{
		RTC_DCHECK(audio);
#if defined(WEBRTC_NS_FLOAT)
		rtc::CritScope cs(crit_);
	...
	void VS_NoiseSuppressionImpl::ProcessCaptureAudio(AudioBuffer* audio)
	{
		RTC_DCHECK(audio);
		rtc::CritScope cs(crit_);
		if (!enabled_)
	...
	int VS_NoiseSuppressionImpl::Enable(bool enable)
	{
		rtc::CritScope cs(crit_);
		if (enabled_ != enable)
		{
	...
	bool VS_NoiseSuppressionImpl::is_enabled() const
	{
		rtc::CritScope cs(crit_);
		return enabled_;
	}

	int VS_NoiseSuppressionImpl::set_level(Level level)
	{
		int policy = 1;
		switch (level)
		{
	...
	NoiseSuppression::Level VS_NoiseSuppressionImpl::level() const
	{
		rtc::CritScope cs(crit_);
		return level_;
	}

	float VS_NoiseSuppressionImpl::speech_probability() const
	{
		rtc::CritScope cs(crit_);
#if defined(WEBRTC_NS_FLOAT)
	...
	std::vector<float> VS_NoiseSuppressionImpl::NoiseEstimate()
	{
		rtc::CritScope cs(crit_);
	...
	size_t VS_NoiseSuppressionImpl::num_noise_bins()
	{
#if defined(WEBRTC_NS_FLOAT)
