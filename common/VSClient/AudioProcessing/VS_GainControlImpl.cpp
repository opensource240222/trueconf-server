...
namespace webrtc
{
	...
	class VS_GainControlImpl::GainController
	{
	public:
	...
	VS_GainControlImpl::VS_GainControlImpl(rtc::CriticalSection* crit_render,
		...
		minimum_capture_level_(1),
		maximum_capture_level_(1000),
#endif
		limiter_enabled_(true),
		target_level_dbfs_(3),
		compression_gain_db_(3),
	...
	VS_GainControlImpl::~VS_GainControlImpl() {}

	int VS_GainControlImpl::ProcessRenderAudio(AudioBuffer* audio)
	{
	...
	void VS_GainControlImpl::ReadQueuedRenderData()
	{
	...
	int VS_GainControlImpl::AnalyzeCaptureAudio(AudioBuffer* audio)
	{
		rtc::CritScope cs(crit_capture_);
	...
	int VS_GainControlImpl::ProcessCaptureAudio(AudioBuffer* audio,
	...
	int VS_GainControlImpl::compression_gain_db() const
	{
		rtc::CritScope cs(crit_capture_);
	...
	int VS_GainControlImpl::set_stream_analog_level(int level)
	{
		rtc::CritScope cs(crit_capture_);
	...
	int VS_GainControlImpl::stream_analog_level()
	{
		rtc::CritScope cs(crit_capture_);
	...
	int VS_GainControlImpl::Enable(bool enable)
	{
	...
	bool VS_GainControlImpl::is_enabled() const
	{
		rtc::CritScope cs(crit_capture_);
		return enabled_;
	}

	int VS_GainControlImpl::set_mode(Mode mode)
	{
		rtc::CritScope cs_render(crit_render_);
		rtc::CritScope cs_capture(crit_capture_);
	...
	GainControl::Mode VS_GainControlImpl::mode() const
	{
		rtc::CritScope cs(crit_capture_);
		return mode_;
	}

	int VS_GainControlImpl::set_analog_level_limits(int minimum,
		int maximum)
	{
		if (minimum < 0)
		{
	...
	int VS_GainControlImpl::analog_level_minimum() const
	{
		rtc::CritScope cs(crit_capture_);
		return minimum_capture_level_;
	}

	int VS_GainControlImpl::analog_level_maximum() const
	{
		rtc::CritScope cs(crit_capture_);
		return maximum_capture_level_;
	}

	bool VS_GainControlImpl::stream_is_saturated() const
	{
		rtc::CritScope cs(crit_capture_);
		return stream_is_saturated_;
	}

	int VS_GainControlImpl::set_target_level_dbfs(int level)
	{
		if (level > 31 || level < 0)
		{
			return AudioProcessing::kBadParameterError;
	...
	int VS_GainControlImpl::target_level_dbfs() const
	{
		rtc::CritScope cs(crit_capture_);
		return target_level_dbfs_;
	}

	int VS_GainControlImpl::set_compression_gain_db(int gain)
	{
		if (gain < 0 || gain > 90)
		{
			return AudioProcessing::kBadParameterError;
	...
	int VS_GainControlImpl::enable_limiter(bool enable)
	{
		{
			rtc::CritScope cs(crit_capture_);
	...
	bool VS_GainControlImpl::is_limiter_enabled() const
	{
		rtc::CritScope cs(crit_capture_);
		return limiter_enabled_;
	}

	void VS_GainControlImpl::Initialize(size_t num_proc_channels, int sample_rate_hz)
	{
		rtc::CritScope cs_render(crit_render_);
		rtc::CritScope cs_capture(crit_capture_);
	...
	void VS_GainControlImpl::AllocateRenderQueue()
	{
		rtc::CritScope cs_render(crit_render_);
		rtc::CritScope cs_capture(crit_capture_);
	...
	int VS_GainControlImpl::Configure()
	{
		rtc::CritScope cs_render(crit_render_);
		rtc::CritScope cs_capture(crit_capture_);
