...
void erase(std::forward_list<T, Allocator>& c, const U& value)
{
	c.remove_if([&value](const T& x) { return x == value; });
}

template<class T, class Allocator, class Predicate>
void erase_if(std::forward_list<T, Allocator>& c, Predicate pred)
{
	c.remove_if(pred);
}
