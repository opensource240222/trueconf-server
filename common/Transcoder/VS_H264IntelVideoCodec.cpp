...
VS_H264IntelVideoCodec::VS_H264IntelVideoCodec(int CodecId, bool IsCoder) : VideoCodec(CodecId, IsCoder)
{
	m_valid = false;
	m_pmfxEncoder = 0;
	m_pmfxDecoder = 0;
...
VS_H264IntelVideoCodec::~VS_H264IntelVideoCodec()
{
...
void VS_H264IntelVideoCodec::ResetExtOptions()
{
	...
	m_EncExtParams.push_back((mfxExtBuffer*)&m_CodingExtOption);
	// coding ext opt 2
	memset(&m_CodingExtOption2, 0, sizeof(m_CodingExtOption2));
	m_CodingExtOption2.Header.BufferId = MFX_EXTBUFF_CODING_OPTION2;
	m_CodingExtOption2.Header.BufferSz = sizeof(m_CodingExtOption2);
	if (m_maxSliceSize > 0) {
		m_CodingExtOption2.MaxSliceSize = m_maxSliceSize;
		m_EncExtParams.push_back((mfxExtBuffer*)&m_CodingExtOption2);
	}
}

mfxStatus VS_H264IntelVideoCodec::AllocSurfaces(bool bCoder)
{
...
void VS_H264IntelVideoCodec::DeleteSurfaces()
{
...
int VS_H264IntelVideoCodec::Init(int w, int h, uint32_t ColorMode, unsigned char sndLvl, int numThreads, unsigned int framerate)
{
	mfxStatus sts = MFX_ERR_NONE;
	...
	if (sts == MFX_ERR_NONE) {
		...
		if (sts == MFX_ERR_NONE) {
			...
			m_mfxParams.mfx.CodecId = MFX_CODEC_AVC;
			...
			if (IsCoder()) {
				...
				key.GetValue(&level_idc, 4, VS_REG_INTEGER_VT, "H.264 Level IDC");
				m_mfxParams.mfx.TargetUsage = MFX_TARGETUSAGE_BALANCED;
				m_mfxParams.mfx.EncodedOrder = 0;
				m_mfxParams.mfx.NumThread = 0;
				m_mfxParams.mfx.CodecProfile = MFX_PROFILE_AVC_BASELINE;
				...
				m_mfxParams.mfx.NumSlice = (impl == MFX_IMPL_SOFTWARE) ? 0 : h / 16;
				...
				m_mfxParams.mfx.FrameInfo.Width = (w + 15) & ~15;
				m_mfxParams.mfx.FrameInfo.Height = (h + 15) & ~15;
				...
				if (!m_EncExtParams.empty()) {
					m_mfxParams.NumExtParam = (mfxU16)m_EncExtParams.size();
					m_mfxParams.ExtParam = &m_EncExtParams[0];
				}
				///
				mfxVideoParam inPar;
...
void VS_H264IntelVideoCodec::Release()
{
	m_valid = false;
	m_bitrate = 1000;
...
bool VS_H264IntelVideoCodec::UpdateBitrate()
{
	...
	if (!m_EncExtParams.empty()) {
		inPar.NumExtParam = (mfxU16)m_EncExtParams.size();
		inPar.ExtParam = &m_EncExtParams[0];
	}
	inPar.mfx.TargetKbps = (m_bitrate_prev == 1000) ? 1001 : (mfxU16)m_bitrate_prev;
	inPar.mfx.MaxKbps = 3 * inPar.mfx.TargetKbps / 2;
	sts = m_pmfxEncoder->Query(&inPar, &m_mfxParams);
	if (sts >= MFX_ERR_NONE) {
...
int VS_H264IntelVideoCodec::Convert(uint8_t *in, uint8_t* out, VS_VideoCodecParam* param)
{
	int ret = 0;
...
bool VS_H264IntelVideoCodec::SetCoderOption(void *param)
{
	...
	inPar.mfx.CodecProfile = (inpar->cabac_init_idc > 0) ? MFX_PROFILE_AVC_MAIN : MFX_PROFILE_AVC_BASELINE;
	sts = m_pmfxEncoder->Query(&inPar, &m_mfxParams);
	if (sts >= MFX_ERR_NONE) {
		sts = m_pmfxEncoder->Reset(&m_mfxParams);
		sts = m_pmfxEncoder->GetVideoParam(&m_mfxParams);
