...
namespace tc
{
	ldap_error_code_t LDAPCore_OpenLDAP::GetGroupUsers(const char* group_dn, std::vector<ldap_user_info>& group_users, page_cookie_t& cookie,
	...
	ldap_error_code_t LDAPCore_OpenLDAP::GetAllGroupsOfUser(const tc::ldap_user_info& user, const unsigned long user_primary_group_id, const std::string& objectSid, std::set<tc::group_dn>& ret) {
